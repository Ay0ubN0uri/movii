...
export const Genre = () => {
  ...
  const genreList = useAppSelector(state => state.movii.genre.entities);
  const loading = useAppSelector(state => state.movii.genre.loading);
  const links = useAppSelector(state => state.movii.genre.links);
  ...
  return (
    <div>
      <h2 id="genre-heading" data-cy="GenreHeading">
        <Translate contentKey="moviiApp.movieGenre.home.title">Genres</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="moviiApp.movieGenre.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/genre/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="moviiApp.movieGenre.home.createLabel">Create new Genre</Translate>
          </Link>
        </div>
      </h2>
          ...
          {genreList && genreList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="moviiApp.movieGenre.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                ...
                {genreList.map((genre, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      {genre.icon ? (
                        <div>
                          {genre.iconContentType ? (
                            <a onClick={openFile(genre.iconContentType, genre.icon)}>
                              <img src={`data:${genre.iconContentType};base64,${genre.icon}`} style={{ maxHeight: '30px' }} />
                              &nbsp;
                            </a>
                          ) : null}
                          <span>
                            {genre.iconContentType}, {byteSize(genre.icon)}
                          </span>
                        </div>
                      ) : null}
                    </td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="moviiApp.movieGenre.home.notFound">No Genres found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
