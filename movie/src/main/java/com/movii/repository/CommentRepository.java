...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
        value = "select comment from Comment comment left join fetch comment.movie",
        countQuery = "select count(comment) from Comment comment"
    )
    Page<Comment> findAllWithToOneRelationships(Pageable pageable);

    @Query("select comment from Comment comment left join fetch comment.movie")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.movie where comment.id =:id")
